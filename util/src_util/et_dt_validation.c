 ...
 **/
/* Causes as segfault, likely because the key length is importerly set (should be set to 32 as in TEST_1)
** Keeping for now so we can hopefully find a way to catch the segfault and exit gracefullyisher
*/
...
//     // Insert key into keyring of SA 9
//     hex_conversion(buffer_nist_key_h, (char **)&buffer_nist_key_b, &buffer_nist_key_len);
//     memset(ek_ring[test_association->akid].value, 0x00, Crypto_Get_ACS_Algo_Keylen(*test_association->acs));
//     memcpy(ek_ring[test_association->akid].value, buffer_nist_key_b, buffer_nist_key_len);
