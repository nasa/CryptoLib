...
To connect using server-side TLS (1-way TLS, like most websites) from the command line client:
...
UTEST(MARIA_DB_CONNECTION_TESTS, TLS_TEST) {
    printf("START mariadb connection, 1 way TLS test() \n");
    int status = 0;
    ...
    ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
    printf("END mariadb connection, 1 way TLS test() status:%d \n", status);
}
...
int32_t Crypto_Init_Unit_Test_For_DB(void) {
    ...
    Crypto_Config_CryptoLib(SADB_TYPE_MARIADB, CRYPTO_TC_CREATE_FECF_TRUE, TC_PROCESS_SDLS_PDUS_TRUE, TC_HAS_PUS_HDR,
