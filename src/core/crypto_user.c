...
{
    // Local variables
    uint16_t kid = ((uint8_t)sdls_resp_pkt.pdu.data[0] << 8) | ((uint8_t)sdls_resp_pkt.pdu.data[1]);
    uint8_t mod = (uint8_t)sdls_resp_pkt.pdu.data[2];
    ...
    case 2: // Modify key state
        ekp->key_state = (uint8_t)sdls_resp_pkt.pdu.data[3] & 0x0F;
 ...
 * Modifies tm_sec_header.spi based on sdls_resp_pkt.pdu.data[0]
 * @return int32: Success/Failure
 **/
int32_t Crypto_User_ModifyActiveTM(void)
{
    // TODO Check this
    tm_frame_sec_hdr.spi = (uint8_t)sdls_resp_pkt.pdu.data[0];
    return CRYPTO_LIB_SUCCESS;
}
...
{
    // tm_frame.tm_header.vcid = (uint8_t)sdls_resp_pkt.pdu.data[0];
    // Check this
    tm_frame_pri_hdr.vcid = (uint8_t)sdls_resp_pkt.pdu.data[0];
