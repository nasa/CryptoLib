...
UTEST(PERFORMANCE, LSA_LIBG_AUTH_SHORT_100)
{
    ...
    Crypto_Init();
    char *data_h = "202C046100ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF012"
                   "3456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEFF01C";
    char *data_b = NULL;
    int   data_l = 0;
    ...
    float ttl_time_lsa_libg =
        Apply_Security_Loop((uint8_t *)data_b, data_l, ptr_enc_frame, &enc_frame_len, num_frames_100);

    if (ttl_time_lsa_libg < 0)
...
UTEST(PERFORMANCE, MDB_LIBG_AUTH_SHORT_100)
{
    ...
    float ttl_time_mdb_libg =
        Apply_Security_Loop((uint8_t *)data_b, data_l, ptr_enc_frame, &enc_frame_len, num_frames_100);

    if (ttl_time_mdb_libg < 0)
        status = CRYPTO_LIB_ERROR;
    ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
...
UTEST(PERFORMANCE, LSA_KMC_AUTH_SHORT_100)
{
    ...
    float ttl_time_lsa_kmc =
        Apply_Security_Loop((uint8_t *)data_b, data_l, ptr_enc_frame, &enc_frame_len, num_frames_100);

    if (ttl_time_lsa_kmc < 0)
        status = CRYPTO_LIB_ERROR;
    ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
...
UTEST(PERFORMANCE, LSA_LIBG_AUTH_SHORT_1K)
{
    ...
    float ttl_time_lsa_libg =
        Apply_Security_Loop((uint8_t *)data_b, data_l, ptr_enc_frame, &enc_frame_len, num_frames_1K);
