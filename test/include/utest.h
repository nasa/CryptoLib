...
#ifdef _MSC_VER
#define UTEST_SURPRESS_WARNING_BEGIN __pragma(warning(push)) __pragma(warning(disable : 4127))
...
int                     utest_main(int argc, const char *const argv[])
{
    ...
    for (index = 0; index < utest_state.tests_length; index++)
    {
        ...
        printf("%s[ RUN      ]%s %s\n", colours[U_GREEN], colours[U_RESET], utest_state.tests[index].name);
    ...
    printf("%s[==========]%s %" UTEST_PRIu64 " test cases ran.\n", colours[U_GREEN], colours[U_RESET], ran_tests);
    printf("%s[  PASSED  ]%s %" UTEST_PRIu64 " tests.\n", colours[U_GREEN], colours[U_RESET], ran_tests - failed);
