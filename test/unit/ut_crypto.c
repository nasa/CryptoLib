...
UTEST(CRYPTO_C, BAD_CC_FLAG)
{
    ...
    // TC_NO_SEGMENT_HDRS, TC_OCF_NA, 1024, AOS_NO_FHEC, AOS_NO_IZ, 0);
    TCGvcidManagedParameters_t TC_UT_Managed_Parameters = {0, 0x0003, 0, TC_NO_FECF, TC_NO_SEGMENT_HDRS, 1024, 1};
...
UTEST(CRYPTO_C, STRUCT_SIZE_VERIFICATION)
{
    ...
    ASSERT_EQ(AOS_FRAME_PRIMARYHEADER_SIZE, (size_t)13);
    ASSERT_EQ(AOS_FRAME_SECHEADER_SIZE, (size_t)71);
    ASSERT_EQ(AOS_FRAME_SECTRAILER_SIZE, (size_t)MAC_SIZE + 8);
    ASSERT_EQ(AOS_SIZE, (size_t)(13 + 71 + 1786 + MAC_SIZE + 8 + 2));
}
