...
UTEST(EP_SA_MGMT, SA_CREATE_60)
{
    ...
    GvcidManagedParameters_t TC_0_Managed_Parameters = {
        0, 0x0003, 7, TC_NO_FECF, AOS_FHEC_NA, AOS_IZ_NA, 0, TC_HAS_SEGMENT_HDRS, 86, TC_OCF_NA, 1};
    Crypto_Config_Add_Gvcid_Managed_Parameters(TC_0_Managed_Parameters);
    ...
    // Modify SA 0
    sa_if->sa_get_from_spi(63, &test_association);
    test_association->shivf_len      = 0;
    test_association->sa_state       = SA_OPERATIONAL;
    test_association->iv_len         = 12;
    test_association->shsnf_len      = 0;
    ...
    test_association->arsn_len       = 0;
    test_association->gvcid_blk.scid = SCID & 0x3FF;
    test_association->gvcid_blk.vcid = 7;

    // Convert frames that will be processed
    hex_conversion(buffer_CREATE_h, (char **)&buffer_CREATE_b, &buffer_CREATE_len);

    status = Crypto_TC_ProcessSecurity(buffer_CREATE_b, &buffer_CREATE_len, &tc_nist_processed_frame);
    ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);

    printf("\n");
    Crypto_Shutdown();

    free(buffer_CREATE_b);
}
