...
UTEST(TC_PROCESS, TC_PROCESS_IGNORE_SA_STATE)
{
    ...
    // Test string
    char    *test_frame_pt_h   = "2003002719C00004000000000000000000000001ABBA571508526C3B9ED27B938C1B7B5F2343FECF";
    uint8_t *test_frame_pt_b   = NULL;
    int      test_frame_pt_len = 0;
    ...
    // Test string
    test_frame_pt_h   = "2003002719C00004000000000000000000000001ABBA571508526C3B9ED27B938C1B7B5F2343FECF";
...
UTEST(TC_PROCESS, IV_REUSE)
{
    ...
    // 000000000000000000001350

    // Test string              //2003002719C00004000000000000000000000001ABBA571508526C3B9ED27B938C1B7B5F2343FECF
    char    *test_frame_pt_h   = "2003002719C000040000000000000000000000013509e78a6ab914dff4d12ea4185622993f2120DD";
    uint8_t *test_frame_pt_b   = NULL;
    int      test_frame_pt_len = 0;
...
UTEST(TC_PROCESS, SA_CREATE_SPI_OOB_AND_VALID)
{
    remove("sa_save_file.bin");
    Crypto_Config_CryptoLib(KEY_TYPE_INTERNAL, MC_TYPE_INTERNAL, SA_TYPE_INMEMORY, CRYPTOGRAPHY_TYPE_LIBGCRYPT,
                            IV_INTERNAL);
    ...
    TC_t tc_frame;
    memset(&tc_frame, 0, sizeof(tc_frame));
    tc_frame.tc_sec_header.spi = 0;
    memset(sdls_frame.tlv_pdu.data, 0, sizeof(sdls_frame.tlv_pdu.data));

    // SPI == NUM_SA is out of range for sa_create
    sdls_frame.tlv_pdu.data[0] = (uint8_t)((NUM_SA >> 8) & 0xFF);
    sdls_frame.tlv_pdu.data[1] = (uint8_t)(NUM_SA & 0xFF);
    status                     = sa_if->sa_create(&tc_frame);
    ASSERT_EQ(CRYPTO_LIB_ERR_SPI_INDEX_OOB, status);

    // Valid SPI still takes the create path (minimal zeroed PDU payload)
    sdls_frame.tlv_pdu.data[0] = 0x00;
    sdls_frame.tlv_pdu.data[1] = 0x01;
    status                     = sa_if->sa_create(&tc_frame);
    ASSERT_EQ(CRYPTO_LIB_SUCCESS, status);
