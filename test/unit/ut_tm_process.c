...
UTEST(TM_PROCESS_SECURITY, NO_CRYPTO_INIT)
{
    ...
    // status = sa_routine->sa_get_operational_sa_from_gvcid(((uint8_t)framed_tm_b[0] & 0xC0) >> 6, 
    //                                                 (((uint8_t)framed_tm_b[0] & 0x03) << 8) | (uint8_t)framed_tm_b[1],
...
UTEST(TM_PROCESS_SECURITY, NO_CONFIG)
{
    ...
    // printf("STATUS is %d\n", status);
    // Determine security association by GVCID, which nominally happens in TO
    // status = sa_routine->sa_get_operational_sa_from_gvcid(tm_frame_pri_hdr.tfvn, tm_frame_pri_hdr.scid, tm_frame_pri_hdr.vcid, map_id, &sa_ptr);
    // printf("STATUS is %d\n", status);
    status = Crypto_TM_ProcessSecurity((uint8_t* )framed_tm_h, framed_tm_len, &ptr_processed_frame, &processed_tm_len);
...
UTEST(TM_PROCESS_SECURITY, SECONDARY_HDR_PRESENT_MAC)
{
    ...
    // Configure SA 12
    sa_routine->sa_get_sa_from_spi(12, &sa_ptr);
